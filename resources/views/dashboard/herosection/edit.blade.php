...
<div class="px-4 py-5">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
        <div class="flex items-center mb-4 md:mb-0">
            {{-- <a href="{{ route('herosection.show', $hero_section->id) }}" class="mr-3 text-gray-500 hover:text-gray-700 transition-colors">
    ...
    <div class="bg-white rounded-xl shadow-md overflow-hidden border border-gray-100">
        <div class="bg-gradient-to-r from-amber-500 to-amber-600 px-6 py-4">
            <h2 class="text-lg font-semibold text-white flex items-center">
                ...
                Edit Data Hero Section: {{ $hero_section->header }}
            </h2>
            <p class="text-amber-100 text-sm mt-1">Perbarui data hero section dengan informasi terbaru</p>
        </div>
        <div class="p-6">
            <form action="{{ route('herosection.update', $hero_section->id) }}" method="POST">
                @csrf
                @method('PUT')
